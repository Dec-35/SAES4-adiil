...
router.get('', async (req, res) => {
  try {
    ...
    const [productSales] = await pool.query(
      'SELECT * FROM transactionContent JOIN transaction ON transactionContent.transaction_id = transaction.transaction_id RIGHT JOIN product ON transactionContent.product_id = product.id WHERE is_available = 1 ORDER BY purchase_date DESC'
    );
